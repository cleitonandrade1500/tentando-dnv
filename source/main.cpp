...
// Definições para o compilador Clang no PS4
typedef unsigned long size_t;
#define NULL 0

// EXPORTAÇÃO OBRIGATÓRIA PARA GOLDHEN/FPKG
extern "C" {
...
void ProcessarAssalto() {
    ...
    // TESTE DE INJEÇÃO: Vida regenera instantaneamente (God Mode)
    ENTITY::SET_ENTITY_HEALTH(pP, 200, 0);

    // SISTEMA DE MÁSCARA E POLÍCIA (Anti-Detecção)
    ...
    // ABERTURA DE PORTAS AUTOMÁTICA (Ao Mirar)
    if (PLAYER::GET_ENTITY_PLAYER_IS_FREE_AIMING_AT(p, NULL)) {
        Hash portas[] = { 0x7C030E57, 0x5D35D3D1, 0x242D7D7D, 0x4894379D, 0x5395A642 };
        for(int i = 0; i < 5; i++) {
            Object d = OBJECT::GET_CLOSEST_OBJECT_OF_TYPE(pos.x, pos.y, pos.z, 35.0f, portas[i], false, false, true);
...
// ENTRY POINT QUE O GOLDHEN ENXERGA
extern "C" int module_start(size_t argc, const void* args) {
    // Delay de 45 segundos para estabilizar com o Havana Mod
    for(int i = 0; i < 450; i++) { 
        Aguardar(100); 
    }
